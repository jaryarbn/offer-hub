...
export function BankQuestionSidebar({ bankId, activeQuestionId }: BankQuestionSidebarProps) {
  const [keyword, setKeyword] = useState('')
  const deferredKeyword = useDeferredValue(keyword)
  const { data, isPending, isError, error, refetch } = useBankQuestionList(bankId, deferredKeyword)
    ...
    <aside aria-labelledby="bank-question-list-title" className="min-w-0">
      ...
      {isError ? (
        <div className="py-8 text-center" role="alert">
          ...
          <p className="mt-1 text-xs text-muted-foreground">{error.message || '请稍后重试'}</p>
          <button
            type="button"
              ...
              <li key={question.question_id}>
                <Link
                  ...
                  className={cn(
                    ...
                    isActive ? 'bg-secondary text-secondary-foreground' : 'hover:bg-muted/60'
                  )}
                >
...
function BankQuestionListSkeleton() {
  return (
    <div className="divide-y divide-border" aria-busy="true" aria-label="题目列表加载中">
